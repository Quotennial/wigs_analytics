...
all_final_match_scores = all_match_scoring %>%
  ...
  dplyr::mutate(
    ...
    match_score = case_when(
      ...
      first_match_end == 0 & match_result == "Won" ~ paste0(abs(match_points), " Up"),
      first_match_end == 0 & match_result == "Loss" ~ paste0(abs(match_points), " Dn"),
...
all_hole_scoring_by_team = all_match_scoring %>%
  dplyr::select(round, group_id, team_name, team_id, hole_no, hole_par, team_hole_score_nett) %>%
  distinct() %>%
  dplyr::mutate(
    team_hole_score_nett_to_par = team_hole_score_nett-hole_par
  ) %>%
  group_by(round, group_id, team_name, team_id) %>%
  arrange(hole_no) %>%
  dplyr::mutate(
    cumsum_nett_score = cumsum(team_hole_score_nett),
    cumsum_nett_score_to_par = cumsum(team_hole_score_nett_to_par)
  ) %>%
  ungroup() %>%
  dplyr::mutate(
    match_id = paste0("Match ", group_id),
    cumsum_nett_score = ifelse(
      is.infinite(cumsum_nett_score), 
      NA,
      cumsum_nett_score
    ),
    cumsum_nett_score_to_par = ifelse(
      is.infinite(cumsum_nett_score_to_par), 
      NA,
      cumsum_nett_score_to_par
    )
  )
...
all_hole_sg_by_player = all_match_scoring %>%
  ...
  group_by(round, hole_no) %>%
  dplyr::mutate(
    field_avg = mean(hole_score_nett_to_par, na.rm = T)
  ) %>%
  ungroup() %>%
  dplyr::mutate(
    sg = field_avg-hole_score_nett_to_par
  )%>%
  dplyr::select(round, player, team_id, hole_no, sg) %>%
  distinct() %>%
  ...
  dplyr::mutate(
    cumsum_sg = ifelse(
      is.infinite(cumsum_sg), 
      NA,
      cumsum_sg
    )
  )

all_round_sg_by_player = all_match_scoring %>%
  dplyr::mutate(
    hole_score_nett_to_par = hole_score_nett-hole_par
  ) %>%
  group_by(round, player, team_id) %>%
  dplyr::summarise(
    holes_played = length(!is.na(hole_score_nett_to_par)),
    nett_score_to_par = sum(hole_score_nett_to_par, na.rm = T)
  ) %>%
  group_by(round) %>%
  dplyr::mutate(
    field_avg = mean(nett_score_to_par, na.rm = T)
  ) %>%
  ungroup() %>%
  dplyr::mutate(
...
all_player_consistency = all_hole_sg_by_player %>%
  group_by(player, team_id) %>%
  dplyr::summarise(
    avg_sg = mean(sg, na.rm = T),
    std_err_sg = std.error(sg, na.rm = T),
