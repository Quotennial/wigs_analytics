    ...
    contribution_by_team = match_scoring %>%
      ...
      dplyr::mutate(
        ...
        contribution = case_when(
          ...
          hole_score_nett == team_hole_score_nett & hole_won == FALSE & tie == TRUE ~ 0.5,
          hole_score_nett == team_hole_score_nett & hole_won == FALSE & tie == FALSE ~ 0,
      ...
      group_by(team_name, group_id, team_id, player) %>%
      dplyr::summarise(
        holes_played = n(),
